...
softmax_filename= 'Models_Softmax/CIFAR10_Softmax_Resnet20.pth.tar'    
filename= 'Models_PCL/CIFAR10_PCL_Resnet20.pth.tar' 
robust_model= 'robust_model.pth.tar'

checkpoint = torch.load(filename)#select model you want to test
...
for i, (adv_img,real_data) in enumerate(zip(adv_loader,test_loader)):
    ...
    adv_acc += torch.sum(model(normalize(adv_img.clone().detach()))[3].argmax(dim=-1) == label).item()
