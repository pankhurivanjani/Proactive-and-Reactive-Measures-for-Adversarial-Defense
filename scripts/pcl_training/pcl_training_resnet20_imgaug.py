...
class Cspace(object):
    ...
    def __call__(self, X):
        X=X.convert('HSV')#L for GreyScale YCbCr
        return X
...
# Loading the Model    
    ...
    criterion_prox_256 = Proximity(num_classes=num_classes, feat_dim=64, use_gpu=use_gpu)
    
    criterion_conprox_1024 = Con_Proximity(num_classes=num_classes, feat_dim=1024, use_gpu=use_gpu)
    criterion_conprox_256 = Con_Proximity(num_classes=num_classes, feat_dim=64, use_gpu=use_gpu)
    ...
    filename= 'Models_Softmax_resnet20/CIFAR10_Softmax_Resnet20_IMGAUG_Dropout.pth.tar'
    ...
    for epoch in range(args.max_epoch):
        ...
        if args.eval_freq > 0 and (epoch+1) % args.eval_freq == 0 or (epoch+1) == args.max_epoch:
            ...
            torch.save(state_, 'Models_PCL_resnet20/CIFAR10_PCL_Resnet20_IMGAUG_Dropout.pth.tar')
...
def train(model, criterion_xent, criterion_prox_1024, criterion_prox_256, 
    ...
    for batch_idx, (data, labels) in enumerate(trainloader):
        data = normalize(torch.from_numpy(np.array(seq(images=data.permute(0,2,3,1))))).permute(0,3,1,2)
        if use_gpu:
