...
# Loading the Model
    ...
    for epoch in range(args.max_epoch):
        ...
        if args.eval_freq > 0 and (epoch+1) % args.eval_freq == 0 or (epoch+1) == args.max_epoch:
            ...
            torch.save(checkpoint, 'Models_Softmax_resnet20/CIFAR10_Softmax_Resnet20_IMGAUG_Affine.pth.tar')
...
#Batch-wise Training
    for batch_idx, (data, labels) in enumerate(trainloader):
        ...
        data = normalize(torch.from_numpy(np.array(seq(images=data)/255).astype('float32'))).permute(0,3,1,2)
        # data = normalize(torch.from_numpy(np.array(seq(images=data.permute(0,2,3,1))))).permute(0,3,1,2)
        # image = data[0].detach().cpu().squeeze()
        # torch.save(image, 'test_img2.pt')
        # exit()
        if use_gpu:
